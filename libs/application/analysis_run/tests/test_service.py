...
ANALYSIS_RUN_ID = UUID("53000000-0000-0000-0000-000000000003")
...
class AnalysisRuns:
    def __init__(self) -> None:
        self.rows: dict[UUID, AnalysisRun] = {}
    ...
    async def save(
        ...
        *,
        ...
        assert analysis_run.version > expected_version
        self.rows[analysis_run.analysis_run_id] = analysis_run
    ...
    async def get_for_analysis_run(
        ...
        *,
        ...
        matches = [
            ...
            if approval.analysis_run_id == analysis_run_id
...
class UnitOfWork:
    def __init__(self) -> None:
        self.analysis_runs = AnalysisRuns()
...
def service(
    ...
    *,
    ...
    # AnalysisRun, approval, then erasure ids. A fixed sequence keeps the
    # assertions readable; running out of it is what the cycle prevents.
    ids = iter((ANALYSIS_RUN_ID, JOB_ID, APPROVAL_ID, *(uuid4() for _ in range(8))))
...
async def test_start_returns_running_before_the_agents_have_run() -> None:
    ...
    assert unit_of_work.jobs.rows[JOB_ID].analysis_run_id == ANALYSIS_RUN_ID
...
async def test_low_confidence_gates_on_a_human_rather_than_publishing() -> None:
    ...
    await application.execute(actor(), ANALYSIS_RUN_ID)
    detail = await application.get(actor(), ANALYSIS_RUN_ID)
...
@pytest.mark.asyncio
async def test_a_viewer_cannot_start_an_analysis_run() -> None:
...
async def test_owner_decision_is_idempotent_and_member_cannot_decide() -> None:
    ...
    await application.execute(actor(), ANALYSIS_RUN_ID)
    started = await application.get(actor(), ANALYSIS_RUN_ID)
...
@pytest.mark.asyncio
async def test_invisible_analysis_run_returns_not_found() -> None:
    with pytest.raises(AnalysisRunNotFoundError):
        await service(UnitOfWork()).get(actor(Role.VIEWER), ANALYSIS_RUN_ID)


async def outcome_of(pipeline: Pipeline) -> AnalysisRunDetail:
    application = service(UnitOfWork(), pipeline=pipeline)
    await application.start(
        actor(), question="Why did EU refunds increase from June to July 2026?"
    )
    await application.execute(actor(), ANALYSIS_RUN_ID)
    return await application.get(actor(), ANALYSIS_RUN_ID)
...
@pytest.mark.asyncio
async def test_a_drafted_analysis_run_stores_the_draft_with_its_state_change() -> None:
    """Committed alongside the AnalysisRun's own change. A reader must never
    ...
    assert unit_of_work.draft_findings.rows[ANALYSIS_RUN_ID] is draft
...
async def test_a_refresh_returns_the_stored_draft_without_rerunning_insight() -> None:
    ...
    let two readers of the same AnalysisRun see different conclusions."""
    unit_of_work = UnitOfWork()
    draft = structured_draft()
    ...
    first = await application.get(actor(), ANALYSIS_RUN_ID)
    second = await application.get(actor(), ANALYSIS_RUN_ID)
...
class WorkFeed:
    ...
    async def append_for_analysis_run(
        ...
        *,
        ...
        self.events.append(
            {"analysis_run_id": analysis_run_id, "kind": kind, "payload": payload}
        )
...
async def test_the_phase_1_path_still_publishes_on_confidence_alone() -> None:
    ...
    AnalysisRun on a contract that did not exist when it ran would be a
...
async def test_re_evaluating_cannot_produce_a_conflicting_decision() -> None:
    """The lifecycle refuses a second evaluation on a terminal AnalysisRun,
...
class Erasures:
    ...
    terminal AnalysisRun may be erased."""
    ...
    async def request(self, *, erasure_id, organization_id, analysis_run_id, category, now):
        ...
        if not self._terminal:
            raise ErasureError(
                "Evidence can only be erased from a terminal AnalysisRun; "
                "this one is running"
            )
        existing = self.completed.get(analysis_run_id)
    ...
    async def erase(self, *, analysis_run_id, category, now):
        ...
        self.completed[analysis_run_id] = done
        return done


async def completed_analysis_run(unit_of_work: UnitOfWork):
    application = service(unit_of_work, pipeline=Pipeline(score=0.91))
    await application.start(
        actor(), question="Why did EU refunds increase from June to July 2026?"
    )
...
async def test_only_owners_and_admins_may_delete_evidence(role: Role) -> None:
    ...
    application = await completed_analysis_run(unit_of_work)

    with pytest.raises(PermissionDeniedError):
        await application.delete_evidence(
            actor(role), analysis_run_id=ANALYSIS_RUN_ID
        )
...
async def test_an_owner_deletes_and_it_is_recorded_once() -> None:
    ...
    await application.delete_evidence(actor(), analysis_run_id=ANALYSIS_RUN_ID)
...
@pytest.mark.asyncio
async def test_a_live_analysis_run_is_a_conflict() -> None:
    unit_of_work = UnitOfWork()
