...
from .ports import AnalysisRunUnitOfWorkFactory
...
class DurableAnalysisRunExecutor(Protocol):
...
class ExecutionJobWorker:
    ...
    def __init__(
        ...
        *,
        ...
        executor: DurableAnalysisRunExecutor,
    ...
    async def run_once(self) -> bool:
        for (
            organization_id
        ) in await self._unit_of_work_factory.bound_organization_ids():
    ...
    async def _execute(self, organization_id: UUID, job: ExecutionJob) -> None:
        if await self._cancellation_requested(organization_id, job.job_id):
            await self._cancel(organization_id, job.job_id)
            return
        renewal = asyncio.create_task(self._renew(organization_id, job.job_id))
        try:
            ...
            if await self._cancellation_requested(organization_id, job.job_id):
                ...
                # goes to the server log, which is not an organization-facing surface.
                _log.exception(
                ...
                await self._record_failure(organization_id, job.job_id, failure)
        else:
            if await self._cancellation_requested(organization_id, job.job_id):
                await self._cancel(organization_id, job.job_id)
            else:
                await self._complete(organization_id, job.job_id)
        finally:
            renewal.cancel()
    ...
    async def _cancel(self, organization_id: UUID, job_id: UUID) -> None:
        ...
        await self._executor.cancel_job(
            organization_id=organization_id,
            analysis_run_id=cancelled.analysis_run_id,
        )
    ...
    async def _record_failure(
        self, organization_id: UUID, job_id: UUID, failure: ExecutionFailure
    ) -> None:
        ...
        if terminal is not None:
            if (
                ...
                await self._executor.fail_job(
                    organization_id=organization_id,
                    analysis_run_id=terminal.analysis_run_id,
                    failure_category=failure.category,
                )

    async def _renew(self, organization_id: UUID, job_id: UUID) -> None:
        while True:
    ...
    def _uow(self, organization_id: UUID):
