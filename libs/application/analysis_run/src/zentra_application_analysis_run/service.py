...
class AnalysisRunService:
    ...
    async def start(
        ...
        *,
        ...
        return await self._detail(
            ...
            analysis_run,
            None,
            fallback_events=analysis_run.events,
            delivered=delivered,
        )
    ...
    async def execute(self, actor: AuthenticatedActor, analysis_run_id: UUID) -> None:
        await self._execute(actor, analysis_run_id, record_failure=True)
    ...
    async def cancel_job(
        ...
        *,
        ...
        never rewrites an AnalysisRun that already reached a terminal state.
        """
        now = self._now()
        async with self._unit_of_work_factory(
            ...
            if analysis_run is None or analysis_run.status in TERMINAL_STATUSES:
                return
            expected_version = analysis_run.version
            analysis_run.cancel(now)
            await unit_of_work.analysis_runs.save(
                analysis_run, expected_version=expected_version
            )
            await unit_of_work.outbox.enqueue(analysis_run.events)
            await unit_of_work.work_feed.append_for_analysis_run(
                ...
                kind=WorkFeedEventKind.ANALYSIS_RUN_CANCELLED,
    ...
    async def cancel(
        ...
        async with self._unit_of_work_factory(
            ...
            await unit_of_work.work_feed.append_for_analysis_run(
                ...
                analysis_run_id=analysis_run_id,
                kind=(
                    WorkFeedEventKind.ANALYSIS_RUN_CANCELLED
                    if job.status is ExecutionJobStatus.CANCELLED
                    else WorkFeedEventKind.ANALYSIS_RUN_CANCEL_REQUESTED
                ),
                payload=AnalysisRunEventPayload(
                    analysis_run_id=analysis_run_id,
                    status=(
                        ...
                        if job.status is ExecutionJobStatus.CANCELLED
                        else analysis_run.status
                    ),
                ),
                occurred_at=now,
            )
            await unit_of_work.commit()
        return await self.get(actor, analysis_run_id)
    ...
    async def retry(
        ...
        async with self._unit_of_work_factory(
            ...
            if original.status not in {
                ...
                raise ConflictError(
                    "Only a failed or cancelled AnalysisRun can be retried"
                )
            sequence = None
            if original.thread_id is not None:
                ...
                if latest is None or latest.analysis_run_id != analysis_run_id:
                    raise ConflictError(
                        "Only the latest Thread AnalysisRun can be retried"
                    )
                sequence = (latest.thread_sequence or 0) + 1
            retried = AnalysisRun.create(
                analysis_run_id=self._new_id(),
                organization_id=actor.organization_id,
                ...
                parent_analysis_run_id=original.parent_analysis_run_id,
                retry_of_analysis_run_id=original.analysis_run_id,
            )
            retried.start(now)
            job = ExecutionJob.create(
                job_id=self._new_id(),
                organization_id=actor.organization_id,
                analysis_run_id=retried.analysis_run_id,
                now=now,
            )
            await unit_of_work.analysis_runs.add(retried)
            await unit_of_work.jobs.add_job(job)
            ...
            if retried.thread_id is not None and hasattr(unit_of_work, "work_feed"):
                await unit_of_work.work_feed.append_for_analysis_run(
                    ...
                    event_id=self._new_id(),
                    analysis_run_id=retried.analysis_run_id,
                    kind=WorkFeedEventKind.ANALYSIS_RUN_RETRY_CREATED,
                    payload=AnalysisRunEventPayload(
                        analysis_run_id=retried.analysis_run_id,
                        status=retried.status,
                        parent_analysis_run_id=retried.parent_analysis_run_id,
                        retry_of_analysis_run_id=original.analysis_run_id,
                    ),
                    occurred_at=now,
                )
        ...
        *,
        ...
        try:
            result = await self._pipeline.run(
                ...
                data_connection_id=analysis_run.data_connection_id,
            )
        except Exception as error:
            if not record_failure:
                raise
            await self._fail(actor, analysis_run, "unexpected")
            raise ScenarioUnavailableError(
                "The analysis_run pipeline could not complete"
            ) from error
        ...
        async with self._unit_of_work_factory(
            ...
            if result.draft_finding is not None:
                # Same transaction as the AnalysisRun's own state change, so
    ...
    async def delete_evidence(
        ...
        *,
        ...
        """Erase a terminal AnalysisRun's evidence, at an Organization's request.
        ...
        async with self._unit_of_work_factory(
            ...
            try:
                ...
                # A live AnalysisRun. Typed, because "not yet" is a different
            ...
            if not already_done:
                cursor = len(analysis_run.events)
                analysis_run.record_evidence_erased(now, category=category.value)
                await unit_of_work.outbox.enqueue(analysis_run.events[cursor:])
            await unit_of_work.commit()
    ...
    async def resolve_citation(
        ...
        *,
        ...
        answer. "Another Organization's", "another AnalysisRun's" and "does not
    ...
    async def decide(
        ...
        *,
        ...
        if actor.role not in {Role.OWNER, Role.ADMIN}:
            await self._record_denial(actor, analysis_run_id, approval_id)
        ...
        async with self._unit_of_work_factory(
            ...
            if analysis_run is None or approval is None:
                raise AnalysisRunNotFoundError("AnalysisRun was not found")
            ...
            try:
                ...
                if changed:
                    expected_version = analysis_run.version
                    event_cursor = len(analysis_run.events)
                    analysis_run.decide(
                        decision=decision,
                        rejection_reason=rejection_reason,
                        now=self._now(),
                    )
                    new_events = analysis_run.events[event_cursor:]
                    await unit_of_work.analysis_runs.save(
                        analysis_run,
                        expected_version=expected_version,
                    )
        ...
        *,
        ...
        return AnalysisRunDetail(
            ...
            can_delete_evidence=(
                ...
                and analysis_run.status in TERMINAL_STATUSES
            ),
    ...
    def _require_create_role(actor: AuthenticatedActor) -> None:
        if actor.role not in {Role.OWNER, Role.ADMIN, Role.MEMBER}:
            raise PermissionDeniedError("This membership cannot start analysis_runs")
    ...
    async def _record_denial(
        ...
        Silent on a missing or invisible AnalysisRun, because recording
        ...
        would let anyone who can read an AnalysisRun generate unbounded audit
        ...
        async with self._unit_of_work_factory(
            ...
            if (
                ...
                return
            cursor = len(analysis_run.events)
            analysis_run.record_denied_decision(
                self._now(),
...
def _publication_decision(
    ...
    *,
    ...
    gating every legacy AnalysisRun on a contract that did not exist when it
