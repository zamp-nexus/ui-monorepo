...
from .ports import AnalysisRunUnitOfWork
...
async def prepare_published_visualization(
    *,
    ...
    if analysis_run.finding is None:
        return None
    ...
    actions = _actions(analysis_run, active, now)
    brief = _brief(analysis_run, draft, active, actions)
    content_hash = brief.content_hash(renderer_configuration=RENDERER_CONFIGURATION)
    brief_id = uuid5(
        _IDENTITY_NAMESPACE,
        f"brief:{analysis_run.analysis_run_id}:{analysis_run.version}:{content_hash}",
    )
...
def _actions(
    ...
    if analysis_run.thread_id is None:
        return (), ()
    ...
    if citations:
        values.append(
            VisualizationActionMapping(
                action_id=uuid5(
                    _IDENTITY_NAMESPACE,
                    f"citation:{analysis_run.analysis_run_id}:{citations[0].citation_id}",
                ),
                organization_id=analysis_run.organization_id,
    ...
    values.append(
        VisualizationActionMapping(
            action_id=uuid5(
                _IDENTITY_NAMESPACE,
                f"continue:{analysis_run.analysis_run_id}:{analysis_run.version}",
            ),
            organization_id=analysis_run.organization_id,
            visualization_id=UUID(int=0),
            thread_id=analysis_run.thread_id,
            ...
            label="Continue this analysis_run",
