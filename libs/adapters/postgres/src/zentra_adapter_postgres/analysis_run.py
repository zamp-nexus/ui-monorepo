...
from zentra_application_analysis_run import AnalysisRunUnitOfWork, UsageSummary
...
class ConcurrentAnalysisRunUpdateError(RuntimeError):
    pass
...
def _state_to_json(analysis_run: AnalysisRun) -> dict[str, Any]:
    outcome = analysis_run.outcome
    return {
        "finding": _finding_to_json(analysis_run.finding),
        "outcome": outcome.model_dump(mode="json") if outcome else None,
        "completion": (
            {"human_approved": analysis_run.completion.human_approved}
            if analysis_run.completion
            else None
        ),
        "failure": (
            {
                "code": analysis_run.failure.code,
                "message": analysis_run.failure.message,
                "category": analysis_run.failure.category,
            }
            if analysis_run.failure
            else None
        ),
        "data_connection_id": (
            str(analysis_run.data_connection_id)
            if isinstance(analysis_run.data_connection_id, UUID)
            else None
        ),
        "data_connection_ids": (
            [str(source_id) for source_id in analysis_run.data_connection_id]
            if isinstance(analysis_run.data_connection_id, tuple)
            else None
        ),
        "source_scope_id": (
...
def _analysis_run_from_row(row: Any) -> AnalysisRun:
    state = row.state or {}
    ...
    data_connection_values = state.get("data_connection_ids")
    ...
    return AnalysisRun(
        ...
        parent_analysis_run_id=row.parent_analysis_run_id,
        ...
        failure=failure,
        data_connection_id=(
            tuple(UUID(value) for value in data_connection_values)
            if data_connection_values
            else UUID(data_connection_value)
            if data_connection_value
            else None
        ),
...
class PostgresAnalysisRunRepository:
    ...
    async def add(self, analysis_run: AnalysisRun) -> None:
        await self._connection.execute(
            insert(analysis_runs).values(
                ...
                retry_of_analysis_run_id=analysis_run.retry_of_analysis_run_id,
    ...
    async def all_for_thread(self, thread_id: UUID) -> tuple[AnalysisRun, ...]:
        ...
        return tuple(_analysis_run_from_row(row) for row in rows)
    ...
    async def save(
        ...
        *,
        ...
        if result.rowcount != 1:
            raise ConcurrentAnalysisRunUpdateError(
                "AnalysisRun was modified by another request"
            )
    ...
    async def add(self, approval: HumanApproval) -> None:
        await self._connection.execute(
            insert(human_approvals).values(
                ...
                analysis_run_id=approval.analysis_run_id,
...
class PostgresAuditOutboxRepository:
    ...
    async def enqueue(self, events: Sequence[DomainEvent]) -> None:
        """Append, keeping each AnalysisRun's timeline strictly increasing.

        The aggregate bumps `occurred_at` by a microsecond when two of its own
        events share an instant, but `_analysis_run_from_row` rehydrates
        ...
        analysis_run_id = events[0].analysis_run_id
        if any(event.analysis_run_id != analysis_run_id for event in events):
            raise ValueError("One enqueue carries one AnalysisRun's events")
    ...
    async def pending(
        ...
        *,
        ...
        if analysis_run_id is not None:
            query = query.where(audit_outbox.c.analysis_run_id == analysis_run_id)
...
class PostgresAnalysisRunUnitOfWork(AnalysisRunUnitOfWork):
    def __init__(
        self,
        connection: AsyncConnection,
        *,
        ...
        self.analysis_run_boards = PostgresAnalysisRunBoardRepository(connection)
...
class PostgresAnalysisRunUnitOfWorkFactory:
    ...
    async def __call__(
        ...
        trace_id: UUID,
        span_id: UUID,
    ) -> AsyncIterator[PostgresAnalysisRunUnitOfWork]:
        async with self._database.engine.connect() as connection:
            transaction = await connection.begin()
            await set_organization_context(connection, organization_id)
            unit_of_work = PostgresAnalysisRunUnitOfWork(
                connection,
                trace_id=trace_id,
                span_id=span_id,
            )
