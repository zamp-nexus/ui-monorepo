...
async def test_transactional_lifecycle_outbox_rls_and_idempotent_approval() -> None:
    ...
    await service.execute(actor, started.analysis_run_id)
    started = await service.get(actor, started.analysis_run_id)

    assert started.status is AnalysisRunStatus.AWAITING_APPROVAL
    ...
    assert repeated.status is AnalysisRunStatus.COMPLETED
