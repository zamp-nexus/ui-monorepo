...
node (`test_graph.py`). ADR-0023 deleted the graph and `OrchestratorLoop` does
...
async def test_a_missing_required_role_refuses_rather_than_proceeding() -> None:
    planner = agent(advertised=(AgentRole.SQL_ANALYST,))
...
async def test_it_refuses_when_insight_is_required_but_not_promoted() -> None:
    ...
    planner = agent(advertised=(AgentRole.SQL_ANALYST, AgentRole.EVALUATOR))
...
async def test_its_output_is_a_task_ledger_and_nothing_else() -> None:
    ...
    planner = agent(
        advertised=(AgentRole.SQL_ANALYST, AgentRole.EVALUATOR, AgentRole.INSIGHT)
    )
