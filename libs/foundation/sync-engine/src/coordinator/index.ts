...
import { NetworkStatusMonitor, createNetworkMonitor } from '../network';
import { OfflineQueueManager, createQueueManager, QueueProcessor } from '../queue';
import { ConflictResolver, createConflictResolver } from '../conflicts';
import { ConvexSyncAdapter, createConvexAdapter, type ConvexMutationOptions } from '../convex';
import { CrossTabManager, createCrossTabManager } from '../cross-tab';
import type { SyncEvent, SyncEventListener } from '@open-insights-web/foundation-data-model';
...
export class SyncCoordinator extends AsyncDisposable implements ISyncCoordinator {
  ...
  constructor(config: SyncCoordinatorConfig) {
    ...
    // Note: autoStart is handled by createAndStartSyncCoordinator() factory
    // Constructor no longer has side effects (no setTimeout)
  }
  ...
  async start(): Promise<void> {
    ...
    this.disposables.addFunction(networkUnsubscribe);
  ...
  async sync(): Promise<ProcessingResult | null> {
    ...
    try {
      ...
      this.emit({ type: SyncEventType.SYNC_START, timestamp: Date.now() });
      ...
      this.emit({
        type: SyncEventType.SYNC_COMPLETE,
        timestamp: Date.now(),
        data: {
          queueResult: result,
  ...
  private handleNetworkStatusChange = (status: NetworkStatus | { isOnline: boolean }): void => {
    ...
    if (status.isOnline) {
      ...
      this.emit({ type: SyncEventType.ONLINE, timestamp: Date.now() });
      ...
      this.emit({ type: SyncEventType.OFFLINE, timestamp: Date.now() });
  ...
  private setupCrossTabHandlers(): void {
    ...
    const invalidateUnsub = this.crossTabManager.subscribe(CrossTabMessageType.INVALIDATE, (message) => {
    ...
    this.disposables.addFunction(invalidateUnsub);

    // Handle mutation completion from other tabs
    const mutationUnsub = this.crossTabManager.subscribe(CrossTabMessageType.MUTATION_COMPLETED, (message) => {
    ...
    this.disposables.addFunction(mutationUnsub);

    // Handle online status from other tabs
    const onlineUnsub = this.crossTabManager.subscribe(CrossTabMessageType.ONLINE, () => {
    ...
    this.disposables.addFunction(leaderUnsub);
  }
...
/**
 * Create a sync coordinator instance without starting it.
 * Call start() manually to begin synchronization.
 *
 * For automatic start, use createAndStartSyncCoordinator() instead.
 *
 * @param config - Sync coordinator configuration
 * @returns SyncCoordinator instance (not started)
 */
export const createSyncCoordinator = (config: SyncCoordinatorConfig): SyncCoordinator =>
  new SyncCoordinator(config);
 ...
 *
 * The autoStart option in config is ignored by this function - it always starts.
 *
 * @param config - Sync coordinator configuration
