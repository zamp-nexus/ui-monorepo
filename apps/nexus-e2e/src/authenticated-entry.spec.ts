...
test('an unauthenticated browser is not signed in by the harness', async ({ anonymous }) => {
  ...
  await expect(anonymous.getByRole('heading', { name: /observatory|investigation/i })).toBeHidden();
});

test('an owner reaches the workspace with a real API identity', async ({ owner }) => {
  const context = owner.waitForResponse(
    (response) => response.url().includes('/v1/context') && response.status() === 200,
  );
  await owner.goto('/');
  ...
  expect(resolved.organization_name).toBe('Forensic Observatory E2E');
});

for (const role of ['owner', 'admin', 'member', 'viewer'] as const) {
  test(`the API resolves ${role} from the token alone`, async ({ page, baseURL }) => {
    ...
    const context = page.waitForResponse((response) => response.url().includes('/v1/context'));
    await page.goto('/');
...
test('the signed-in workspace has no accessibility violations', async ({ owner }) => {
  await owner.goto('/');
  await owner.waitForResponse((response) => response.url().includes('/v1/context'));
