...
const open = async (page: Page, id: string) => {
  const loaded = page.waitForResponse(
    (response) => response.url().includes(`/v1/investigations/${id}`) && response.status() === 200,
  );
...
test.describe.skip('an automatically publishable Investigation', () => {
  ...
  test('labels evidence and interpretation as different things', async ({ owner }) => {
    await open(owner, PUBLISHED);
    ...
    await expect(owner.getByText('Interpretation', { exact: true })).toHaveCount(1);
  });
...
test.describe.skip('an Investigation held back from publication', () => {
  ...
  test('does not conflate unavailable evidence with deleted evidence', async ({ owner }) => {
...
test.describe.skip('nothing a Tenant owns leaks into the page', () => {
  for (const [name, id] of [
    ...
    test(`the ${name} Investigation renders no prompt or hidden reasoning`, async ({ owner }) => {
      await open(owner, id);
      const body = (await owner.locator('body').innerText()).toLowerCase();
...
test.describe.skip('accessibility', () => {
  for (const [name, id] of [
    ...
    test(`the ${name} Investigation has no WCAG A or AA violations`, async ({ owner }) => {
  ...
  test('the evidence disclosure is reachable by keyboard', async ({ owner }) => {
    ...
    const disclosure = owner.getByRole('button', { name: /evidence|citation/i }).first();
