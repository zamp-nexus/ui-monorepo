...
export default defineConfig(({ mode }) => ({
  ...
  cacheDir: '../../node_modules/.vite/apps/nexus',
  server: {
    port: 4200,
    host: 'localhost',
  },
    ...
    alias: [
      ...
      // bypasses the export-map restriction rather than the file itself.
      {
        ...
        replacement: fileURLToPath(
          new URL('./node_modules/@thesysai/genui-sdk/dist/genui-sdk.css', import.meta.url),
        ),
      },
      ...
      ...(mode === 'e2e'
        ? // Exact matches, not prefixes. An object alias matches on prefix and
          ...
          [
            ...
            replacement: fileURLToPath(new URL('./src/e2e/auth-stub.tsx', import.meta.url)),
          }))
        : []),
    ],
  },
  ...
  test: {
    ...
    globals: true,
    environment: 'jsdom',
    setupFiles: ['./src/test/setup.ts', './src/test/xyflow-shims.ts', './src/test/assistant-ui-shims.ts'],
