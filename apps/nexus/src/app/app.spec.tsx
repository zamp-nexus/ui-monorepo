...
const analysisRun = {
  ...
  finding: {
    ...
    summary: 'Governed EU refund amount increased while order volume remained flat.',
    metrics: [
      {
...
describe('App', () => {
  ...
  it('shows persisted evidence and owner approval controls on refresh', async () => {
    ...
    // matches both.
    expect(await screen.findByRole('button', { name: /approve finding/i })).toBeTruthy();
  ...
  const structuredDraft = {
    ...
    contradictions: [{ detail: 'Recheck counted 8 rows, not 12.', resolved: false }],
  ...
  it('says a legacy analysis run predates structured claims', async () => {
    ...
    expect(screen.getByText(/ran before claims were recorded separately/i)).toBeTruthy();
  ...
  it('labels measured claims apart from interpreted ones', async () => {
    ...
    expect(screen.queryByText(/ran before claims were recorded separately/i)).toBeNull();
  });
  ...
  it('surfaces an unresolved contradiction rather than smoothing it away', async () => {
    ...
    expect(screen.getAllByText(/Recheck counted 8 rows, not 12\./)).toHaveLength(2);
  ...
  it('lists every condition the policy found failing', async () => {
    ...
    expect(screen.getByText(/bounded confidence is below the tenant threshold/i)).toBeTruthy();
    expect(screen.getByText(/no evidence that can be followed/i)).toBeTruthy();
  ...
  it('shows what the decision turns on, beside the decision', async () => {
    ...
    expect(screen.getAllByText(/Recheck counted 8 rows, not 12\./).length).toBeGreaterThan(0);
  });
