...
function CitationDisclosure({
  ...
  return (
    <details className={styles.evidence} onToggle={(event) => setOpen(event.currentTarget.open)}>
      <summary>
        Evidence
...
const stateCopy: Record<Exclude<ResolutionState, 'active'>, string> = {
  ...
  unavailable: 'This evidence is unavailable. It was recorded, and cannot currently be reached.',
  tombstoned:
    ...
    <dl>
      ...
      {citation.filters.length > 0 ? (
        <>
          ...
          <dd>
            {citation.filters
              .map((filter) => `${filter.member} ${filter.operator} ${filter.values.join(', ')}`)
              .join('; ')}
          </dd>
        </>
      ) : null}
...
export function DraftFindingPanel({
  ...
  return (
    <section className={styles.draft} data-state="structured" aria-labelledby={headingId}>
      ...
      <ol className={styles.claims}>
        {draft.claims.map((claim) => (
          <li key={claim.claim_id} className={styles.claim}>
            ...
            <Badge intent={claim.kind === 'observed' ? 'success' : 'default'} size="sm">
