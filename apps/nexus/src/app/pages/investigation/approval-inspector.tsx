  ...
  return (
    <dl className="mt-5 grid grid-cols-[auto_1fr] gap-x-4 gap-y-2 text-sm">
      ...
      <dd className="m-0 text-foreground">
        {draft.confidence
          ? `${Math.round(
              draft.confidence.score * 100,
            )}% · ${draft.confidence.calibration_method.replace(/_/g, ' ')}`
          : 'Not reported'}
      </dd>
      <dt className="font-mono text-[10px] uppercase tracking-[0.14em] text-foreground-muted">
        Measured claims
        ...
        {unavailable.length === 0 && tombstoned.length === 0
          ...
          : [
              unavailable.length > 0
                ...
                : null,
              tombstoned.length > 0 ? `${tombstoned.length} erased at the tenant's request` : null,
            ]
              .filter(Boolean)
              .join(' · ')}
      </dd>
...
export const ApprovalInspector = ({
  ...
  if (!approval) {
    return (
      <aside className="flex flex-col gap-3 border border-border bg-card p-5" aria-live="polite">
