...
export const PlaceholderConfig = ({ connector }: PlaceholderConfigProps) => (
  <section className="px-8 py-10">
    ...
    <Card padding="lg" className="mt-8 max-w-2xl">
      <EmptyState size="lg" icon={<Icon name={connector.icon} size="xl" />}>
        ...
        <EmptyState.Description>
          ...
          {connector.name} means a credential model of its own and a driver behind it, not a form on
          this page.
        </EmptyState.Description>
        <EmptyState.Actions>
    ...
    <Alert intent="info" className="mt-5 max-w-2xl" title="Nothing here is saved">
      This page has no fields on purpose. Credentials typed into a form that posts nowhere are still
      credentials that were typed.
    </Alert>
  </section>
);
