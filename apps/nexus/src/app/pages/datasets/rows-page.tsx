...
export const RowsPage = ({ getToken }: RowsPageProps) => {
  ...
  const rows = useQuery({
    ...
    retry: (count, error) =>
      !(error instanceof ApiError && (error.status === 404 || error.status === 503)) && count < 2,
  });
  ...
  return (
    <section className="px-8 py-10">
      ...
      {notReady ? (
        <Alert intent="info" className="mt-8" role="status" title="Still syncing">
          This dataset isn't ready to browse yet. It may still be syncing — try again in a few
          minutes.
        </Alert>
      ) : null}
