...
export const SequencesPage = ({ getToken, identity }: SequencesPageProps) => {
  ...
  return (
    <section className="px-8 py-10">
      ...
      <div className="mt-3 flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-3 max-w-2xl text-sm text-foreground-muted">
            Multi-step cleaning pipelines from a Raw Table to one or more Final Tables, built by
            chatting with Data Steward.
          </p>
        </div>
      ...
      {sequences.error ? (
        <Alert intent="error" className="mt-10" role="alert" title="Sequences could not be listed">
      ...
      {sequences.data && items.length === 0 ? (
        ...
        >
          ...
          <EmptyState.Description>
            Start one from a Raw Table and Data Steward will build it out as you chat.
          </EmptyState.Description>
          <EmptyState.Actions>
            <Button disabled={!canWrite} onClick={() => setCreating(true)}>
              New Sequence
      ...
      <NewSequenceModal open={creating} getToken={getToken} onClose={() => setCreating(false)} />
    </section>
  );
};
