...
const depthsByPreparedTable = (preparedTables: readonly PreparedTable[]): Map<string, number> => {
...
const failedRunDepth = (run: FailedRun, tableDepths: Map<string, number>): number =>
  run.anchor_prepared_table_id ? (tableDepths.get(run.anchor_prepared_table_id) ?? 0) + 1 : 1;
...
export const layoutSequenceGraph = (graph: SequenceGraph): SequenceFlowGraph => {
  ...
  const stepByProducedTable = new Map(graph.steps.map((step) => [step.produced_table_id, step]));
  ...
  for (const table of graph.prepared_tables) {
    pushEntry(
      tableDepths.get(table.prepared_table_id) ?? 1,
      table.prepared_table_id,
      table.created_at,
    );
  }
